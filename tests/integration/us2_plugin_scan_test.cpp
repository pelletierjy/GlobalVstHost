...
TEST_F(US2PluginScanTest, ScanReturnsEmptyCatalogWhenNoPathsSet)
{
    ...
    // It is never fully empty: the built-in effects (Volume Leveler, EQ, Compressor) are always
...
TEST_F(US2PluginScanTest, CatalogEntriesHaveRequiredFields)
{
    ...
    // Built-in effects (Volume Leveler, EQ, Compressor) are registered directly, not scanned from
